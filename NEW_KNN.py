...
def load_data(train_data, test_data, k, row):
    ...
    print('For K =', k, '\n', new_list)
    print('Dataset Is in Class: ',max(new_list, key=new_list.count)) # Print maximum iterated number in the list to find out the class of the row
    ...
    accu_knn = (max(count_number)/len(new_list))*100 # Calculate the value of Estimated conditional probability of the predicted class
    print('Estimated conditional probability of the predicted class: ', accu_knn, '%')
